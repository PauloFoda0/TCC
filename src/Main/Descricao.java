...
public class Descricao extends Application {
    ...
    public static void setStage(Stage stage) { //Stage publico para selecionar
        Descricao.stage = stage;
    }
    

    @Override
    public void start(Stage stage) throws Exception {
        Parent root = FXMLLoader.load(getClass().getResource("/View/Descricao.fxml")); //Puxa a tela que se localiza no View
        
        Scene scene = new Scene(root);
        
        stage.setScene(scene); //Seleciona tela ao Stage
        stage.setTitle("Descricao"); //Titulo da tela
        stage.getIcons().add(new Image(getClass().getResourceAsStream("/Imagens/Icone produto.png")));
        stage.show(); //Abre a tela
        Descricao.setStage(stage); //Adiciona o Stage a Descricao
    }
